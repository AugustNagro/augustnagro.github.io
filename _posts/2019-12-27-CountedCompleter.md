...
The graph above is very close to what java.util.Stream's AbstractTask accomplishes. But because we operate on an array of fixed size, there are several optimizations that can be employed. First, conider the target number of tasks. It's reasonable to set a value like 4 * #cores, like in AbstractTask. But some problems may benefit from much more, especially if the processing time is irregular. But on the flip side, more sub-tasks means more allocations and greater framework overhead. So it's not a good idea to spawn a task per array item. Additionally, if many jobs are being submitted to the same pool, [ForkJoinTask::getSurplusQueuedTaskCount](https://docs.oracle.com/en/java/javase/13/docs/api/java.base/java/util/concurrent/ForkJoinTask.html#getSurplusQueuedTaskCount()) can be used to dynamically decide whether to spawn sub-tasks. For this implementation, we'll stick to 4 * #cores.
...
Running on a 40 Core Xeon E5-2660 v3 @ 2.60GHz, we see the following results for a random array of size 50 million
