...
With Scala 3, you can easily implement fast C-style loops:
...
The usage looks like this, and we define even easier varients at the end:
...
The generated byte-code can be viewed with the [CFR](https://www.benf.org/other/cfr/) decompiler, and looks exactly how you'd want; a simple Java for loop.
...
But why not use for-do or while? Lets look at one example: given a two dimensional array, return true if it contains some integer x.
...
Numbers are being boxed, Early Return is implemented by throwing Exceptions, and because x is in an outer scope, both foreach lambdas are capturing, creating anonymous classes.

The performance is good with `while`; the byte code looks exactly like the source. But it's not nearly as ergonomic.
